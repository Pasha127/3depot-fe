...
const resultHandler = (otherUser, history, user, onlineUsers) => {
  if (history.length > 0) {
    const existingChats = history.map((chat) => {
      return chat.members;
    });
    /*   console.log("existingChats: ",existingChats) */
    const existingFriends = [];
    existingChats.map((members) => {
      existingFriends.push(members[0]._id);
      existingFriends.push(members[1]._id);
    });
