...
const UserMini = (props) => {
  ...
  useEffect(() => {
    ...
    const users = props.onlineUsers.map((user) => {
      return user._id;
    });
    if (users.includes(props.person._id)) {
      setIsOnline(true);
    } else {
      setIsOnline(false);
    }
  ...
  const chatPreview = () => {
    ...
    if (relevantChat.messages.length) {
      const messagePreview =
        relevantChat.messages[relevantChat.messages.length - 1].content.text;
  ...
  useEffect(() => {
    if (
      props.recentMsg.members.find((member) => member !== props.user._id) ===
      props.thisChat.members.find((member) => member !== props.user._id)
    )
      setChatPreviewLine(props.recentMsg.message?.content.text);
    console.log(
      "recent Chat: ",
      props.recentMsg.members,
      props.user._id,
      props.person._id,
      props.thisChat.members
    );
  ...
  useEffect(() => {
    socket.on("newMessage", (receivedMessage) => {
      ...
      props.person._id === newEntry.sender &&
        setChatPreviewLine(newEntry.content.text);
      props.thisChat.members.some((member) => member._id === newEntry.sender) &&
        setSeenState("tab-body-unseen");
