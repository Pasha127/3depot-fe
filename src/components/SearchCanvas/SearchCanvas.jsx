...
const OBJAsset = () => {
  ...
  return (
    <mesh scale={0.01} position={[0, 0, 0]}>
...
function Box(props) {
  ...
  texture.rotation = pi;
  useEffect(() => {
    if (props.settings.cameraPos !== props.xPos && boxOpen) {
      closeBox();
    }
  ...
  const openBox = () => {
    if (canClick) {
      ...
      setTimeout(
        () => props.setSearchSettings({ activeAsset: props.asset.name }),
        2000
      );
      setClicked(true);
    }
  ...
  return (
    <>
      ...
      >
        ...
        <mesh rotation={[pi / 2, 0, pi]} position={[0, -0.501, 0]}>
          <planeBufferGeometry attach="geometry" args={[1, 1]} />
          <meshStandardMaterial attach="material" map={texture2} transparent />
        </mesh>
        <mesh rotation={[pi / 2, pi, 0]} position={[0, 0.501, 0]}>
          <planeBufferGeometry attach="geometry" args={[1, 1]} />
  ...
  return (
    <>
      <mesh
        ...
        rotation={[pi / 2, 0, 0]}
        onClick={() => {
          navigate(`/Garage?asset=${props.asset._id}`);
        }}
      >
  ...
  return (
    <mesh ref={ref} position={[0, -1, 0]} rotation={[-pi / 2, 0, pi / 2]}>
      ...
      <meshStandardMaterial
        attach="material"
        opacity={0}
        transparent /* map={textureFC} roughnessMap={textureFR} normalMap={textureFN} */
      />
    </mesh>
  );
}
...
function RearPlane(props) {
  ...
  return (
    <mesh position={[props.xPos, -0.2, -1]} rotation={[0, 0, 0]}>
...
function SearchCanvas(props) {
  ...
  useEffect(() => {
    ...
    props.setSearchSettings({ activeAsset: "" });
    if (vw < 500) {
      props.setSearchSettings({ cameraPos: 0 });
    } else if (vw < 1000) {
      props.setSearchSettings({ cameraPos: 2 });
    } else {
      props.setSearchSettings({ cameraPos: 4 });
    }
  }, []);
  useEffect(() => {
  ...
  const ScrollController = () => {
    ...
    useFrame(({ clock }) => {
      camera.position.lerp(
        new Vector3(props.searchSettings.cameraPos, 1, 10),
        0.1
      );
    });
  };
  return (
    <>
        ...
        <Canvas camera={{ fov: 30, position: [0, 0, 10] }}>
          <Suspense fallback={<Loader />}>
            ...
            <Physics>
              ...
              <FloorPlane />
              {props.searchResults &&
                props.searchResults.map((asset, i) => {
      ...
      <DropdownSign />
      {props.searchResults && (
        <ScrollRightTab listLength={props.searchResults.length} />
      )}
