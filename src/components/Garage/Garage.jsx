...
function Garage(props) {
  ...
  useEffect(() => {
    ...
    setUnzippedModel(
      "https://res.cloudinary.com/dirwjcohx/raw/upload/v1674126029/3DepotProducts/3DepotTextBlue_cv6yq6.fbx"
    );
  }, []);
    ...
    <div className="canvas-container">
      <Canvas>
        <Suspense fallback={<Loader />}>
          ...
          <Environment
            background={true}
            files={"industrial_workshop_foundry_bw4.hdr"}
            path={"/"}
          ></Environment>
          <Image />
          <ambientLight intensity={0.3} />
          {props.settings.light && (
            <spotLight
              position={[200, 800, 500]}
              angle={0.3}
              color={`rgb(${props.settings.red},${props.settings.green},${props.settings.blue})`}
              intensity={props.settings.intensity}
            />
          )}
          {props.settings.light && (
            <spotLight
              position={[-600, 800, 500]}
              angle={0.3}
              color={`rgb(${props.settings.red},${props.settings.green},${props.settings.blue})`}
              intensity={props.settings.intensity}
            />
          ...
          {props.settings.axes && (
            <primitive
              object={new THREE.AxesHelper(props.settings.axesSize)}
            ></primitive>
          )}
