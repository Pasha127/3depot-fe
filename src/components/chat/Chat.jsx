...
const Chat = (props) => {
  ...
  const [typingIndicatorClass, setTypingIndicatorClass] = useState(
  ...
  useEffect(() => {
    setOtherUser(
      props.activeChat.members?.find((user) => user._id !== props.user._id)
    );
    setOtherUserName(
      props.activeChat.members
        ?.find((user) => user._id !== props.user._id)
        .email.split("@")[0]
    );
  ...
  const sendMessage = (imgURL = formImageLink) => {
    ...
    const newMessage = {
      members: [
        props.activeChat.members[0]._id,
        props.activeChat.members[1]._id,
      ],
      message: {
    ...
    if (newMessage.message.content.text || newMessage.message.content.media) {
  ...
  const appendNewMsg = (newEntry) => {
    ...
    const duplicateMessage = chatHistory.find(
      (message) =>
        message.createdAt === newEntry.createdAt &&
        message.message === newEntry.message
    );
  ...
  return (
    <Container fluid>
      {props.activeChat && (
        <div className={`convo-header-${props.showHide}`}>
          <div className="convo-header-text">{otherUserName}</div>
        </div>
      )}
      {props.activeChat._id && (
        <Col md={12} className={"chatbar"}>
      ...
      {chatHistory[0] && (
        <Row style={{ height: "95%" }} className="my-3 pe-none">
                          ...
                          <div className={"single-message from-me"}>
                            <ListGroup.Item>
                              <div className="msg-img">
                                ...
                                >
                                  <img
                                    ...
                                    className="img-link-img"
                                    onError={(e) => {
                                      /* if(message.content.media === ""){ */ e.target.className =
                                        "d-none"; /* } */
                              ...
                              <div className="msg-content text-right">
                                {message.content.text?.split("//")[0] ===
                                  "https:" ||
                                message.content.text?.split("//")[0] ===
                                  "http:" ? (
                                  <a href={message.content.text.split(" ")[0]}>
