...
const LogIn = (props) => {
  ...
  useEffect(() => {
    if (awaitingConnection && !props.user._id) {
  ...
  const postNewUser = async (postObj) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Registration Failed - Unique Email, Username and Password are Required"
        );
        setIsLoading(false);
      }
    } catch (error) {
          ...
          <Form className="mt-5 register-box holo-blue-alpha-bg">
            <div className="d-flex justify-content-center">
              <div className="p-0 d-flex pic-space">
                <label className="uploaded-pic" htmlFor="avatarUploadBtn">
                  {!avatar ? (
                    <BsPersonBoundingBox
                      style={{
                        fontSize: "25px",
